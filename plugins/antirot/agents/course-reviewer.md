...
- **Prose quality** — judge against the brief's `voice.exemplar` and `pacing` (`density` + `scaffolding`), **not a generic ideal**:
  ...
  - **Establishes before it uses (the silent on-ramp gap).** Flag any notion the lesson *leans on* as if already-known but never built — not defined here, not in a prereq, no intuition given. The classic case is a **representation shift**: a concept introduced one way (e.g. a term via its inductive grammar) is later used another way (e.g. "the term as a tree, then as a graph") with the new picture *asserted, not established*. This is **invisible to the checker** — the leaned-on notion is plain prose, not a tracked `[[concept]]`, so nothing fires; it is yours to catch. Distinct from a recap (which re-explains the known): this *fails to explain* the relied-upon. A "terms are trees"-style assumption is the exact miss this bullet exists for; do not wave it through as "universal background."
