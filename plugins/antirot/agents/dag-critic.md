...
13. **Undeclared/implicit prerequisites & representation shifts.** The most dangerous prereq is one that isn't even a *concept* yet — a notion a concept's `motivation`/definition/beats lean on that is neither a declared concept nor a listed prereq, so no edge in #1 can catch it and the writer will silently assume it. Read each concept for what it *relies on*. Flag especially **representation shifts**: a concept introduced in one form (a term via its inductive grammar; a function via a formula) and then *used* in another (the term "as a tree, then a graph"; the function "as a curve") — the new representation is itself a teachable thing and must be its own concept/beat or an explicit prereq, not asserted mid-prose. (Real miss this dimension exists for: a `sharing` concept that needs "a term is a syntax tree" while the only upstream lesson taught terms as an inductive grammar — no tree anywhere, so the reader hits an unestablished jump.) For each: name the relying concept, name the relied-upon notion, and propose either promoting it to a concept/beat or adding the explicit prereq edge.
