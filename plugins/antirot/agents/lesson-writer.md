...
4. **Use the notation table verbatim.** If the table fixes a symbol for an object, use exactly that — never a variant (e.g. if it says `\mathbf{x}`, don't write `x` or `vec(x)`). Same object, same symbol, every time.
5. **Imitate the voice exemplar** — register, person, density. You are one author among many; the reader must not feel the seam.
6. **Do not re-teach — and do not *recap* — ALREADY TAUGHT prereqs.** Link them in one clause at the point you use them. **Never open the note (or a section) with a prereq round-up.** Banned openings: "you have already seen…", "you have also already seen…", "as a refresher…", "if you need a refresher…", "we will not re-introduce…". An already-known concept gets no section and no recap paragraph — if you're writing prose *about* a prereq rather than *using* it, delete it and replace with a single `[[link]]` where it first appears. Open the lesson on the problem/the first new concept, not on what the reader already knows.
7. **Obsidian syntax, strictly:** callouts only from the enum `note | info | example | question | tip | warning | summary | success | preview`; LaTeX as `$inline$` / `$$block$$`; flashcards as `Q:: A` lines under `## Flashcards`; reference/transclude a definition with `[[<concept>#^def-<concept>]]` / `![[<concept>#^def-<concept>]]` — the definition lives only in the concept's home lesson (there is no glossary).
