...
2. **Build the DAG.** For each concept, ask: what must the reader already understand for its *definition and first worked example* to make sense? Those are its prereq edges. For every edge set `confidence` honestly (0–1) and a one-line `why` (e.g. "concept B's definition refers to concept A"). Low confidence is information, not weakness — it tells the gate where to look. **No cycles.** The silent killer is a prereq that isn't even a *concept* yet — a notion the definition leans on that you never planned to teach, so no edge can reference it and the writer just assumes it. Watch especially for **representation shifts**: if a concept is taught one way (a term as an inductive grammar; a function as a formula) and later *used* another way (the term "as a tree, then a graph"; the function "as a curve"), the new representation is itself a teachable concept — give it its own home/beat now, or it surfaces later as an asserted, unestablished jump in prose (a `kind="unestablished"` reviewer finding the checker can't see).
...
4. **Lay out notes & modules.** Group into modules, assign a global topological `order` to every note (lower order = taught earlier). Give every note a kebab-case **`slug`** (the stable link ID — it becomes a frontmatter alias and never changes even if the title is reworded) and a human **`title`** (what the reader sees). The file `path` is canonicalized by build-artifacts to `<NN> - <module title>/<NN> - <note title>.md` (filename = title, so Obsidian surfaces it; slug ≠ filename), so don't hand-craft slug-based filenames — give a reasonable path and keep titles distinct within a module.
