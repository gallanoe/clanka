...
- **Motivation is first-class.** Every concept carries a `motivation`: *why it exists* — the problem it solves, or the limitation of a prior concept that demands it ("term rewriting can't express sharing → so we need graphs"). The DAG gives logical order; motivation gives the narrative reason, which order alone can't. Without it, concepts arrive because it's their turn, not because the reader feels the need.
